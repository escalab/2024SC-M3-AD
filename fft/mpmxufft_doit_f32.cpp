...
void setup(double *data, int n, int n_batch)
{
    N = n;
    N_batch = n_batch;
    mpmxufft_f32_Create(&plan, N, N_batch);
    // in_host
    rev = (int *)malloc(sizeof(int) * N);
    gen_rev(N, rev, plan.radices, plan.n_radices);
    in_host = (float *)malloc(sizeof(float) * 2 * N * N_batch);
#pragma omp parallel for
    ...
    cudaMalloc(&in_device_0, sizeof(float) * N * 2 * N_batch);
    cudaMemcpy(in_device_0, in_host, sizeof(float) * N * 2 * N_batch, cudaMemcpyHostToDevice);
}

void finalize(double *result)
{
    cudaMemcpy(in_host, in_device_0, sizeof(float) * N * 2 * N_batch, cudaMemcpyDeviceToHost);
...
void doit(int iter)
{
    for (int t = 0; t < iter; ++t)
        mpmxufft_f32_Exec(plan, in_device_0);
